package com.flowehealth.efr_version.bluetooth.services
...
import com.flowehealth.efr_version.features.configure.advertiser.activities.PendingServerConnectionActivity
import com.flowehealth.efr_version.features.configure.advertiser.services.AdvertiserService
import com.flowehealth.efr_version.features.configure.gatt_configurator.utils.BluetoothGattServicesCreator
import com.flowehealth.efr_version.features.configure.gatt_configurator.utils.GattConfiguratorStorage
import com.flowehealth.efr_version.features.scan.browser.models.logs.*
import com.flowehealth.efr_version.home_screen.activities.MainActivity
import com.flowehealth.efr_version.utils.LocalService
import com.flowehealth.efr_version.utils.Notifications
import com.flowehealth.efr_version.utils.UuidConsts
import timber.log.Timber
import java.lang.reflect.Method
import java.util.*
import com.flowehealth.efr_version.features.scan.browser.models.logs.Log as Log1
...
import com.flowehealth.efr_version.utils.handlePacket
/**
...
class BluetoothService : LocalService<BluetoothService>() {
    ...
    companion object {
        ...
        private const val ACTION_GATT_SERVER_DEBUG_CONNECTION = "com.flowehealth.efr_version.action.GATT_SERVER_DEBUG_CONNECTION"
        private const val ACTION_GATT_SERVER_REMOVE_NOTIFICATION = "com.flowehealth.efr_version.action.GATT_SERVER_REMOVE_NOTIFICATION"
    ...
    private val connectionLogs: MutableList<Log1> = mutableListOf()
    ...
    fun setViewModelfromMain(dataViewModel: DeviceDataViewModel){
    ...
    fun disconnectGatt(deviceAddress: String) {
        Log.d(BuildConfig.TAG, "disconnectGatt: deviceAddress:$deviceAddress")
    ...
    fun addDeviceLog(log: Log1) {
        synchronized(connectionLogs) {
            connectionLogs.add(log)
        }
    }

    fun getLogsForDevice(address: String) : List<Log1> {
    ...
    private val gattCallback: BluetoothGattCallback = object : BluetoothGattCallback() {
        ...
        override fun onServicesDiscovered(gatt: BluetoothGatt, status: Int) {
            ...
            Timber.d("Ble service onServicesDiscovered(): gatt device = ${gatt.device}, status = $status, gatt.services: ${gatt.services}")
            ...
            gatt.services.forEach { service ->
                if (service.uuid.equals(GattService.EPGService.number)) {
                    service.characteristics.forEach { char ->
                        if (char.uuid.equals(GattCharacteristic.EPGNotify.uuid)) {
                            val written = BLEUtils.setNotificationForCharacteristic(gatt, GattService.EPGService, GattCharacteristic.EPGNotify, Notifications.NOTIFY)
                        }
                    }
                }
            }
        ...
        override fun onCharacteristicChanged(gatt: BluetoothGatt, characteristic: BluetoothGattCharacteristic) {
            ...
            Log.d(TAG,"onCharacteristicChanged(): gatt device = ${gatt.device.address}, uuid = ${
                characteristic.uuid}, value = ${data!!.contentToString()}")
            addDeviceLog(GattOperationWithDataLog(gatt, GattOperationLog.Type.CHARACTERISTIC_CHANGED,
                    null, characteristic.uuid, data))
            extraGattCallback?.onCharacteristicChanged(gatt, characteristic)

            handlePacket(data, dataViewModel, gatt.device.address)
        }
...
//    private fun enableNotificationsForCharacteristic(gatt: BluetoothGatt, char: BluetoothGattCharacteristic) {
